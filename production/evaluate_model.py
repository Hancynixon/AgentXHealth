...
# =========================================================
# INLINE extract_prob — no external dependency
# =========================================================
def extract_prob(value):
    ...
    raise ValueError(f"Cannot extract prob from type {type(value)}: {value!r}")


# =========================================================
# ROW-BY-ROW PREDICTOR
# =========================================================
...
for name, auc in all_auc.items():
    if auc >= 0.82:
        status = "✅ Excellent — research grade"
    elif auc >= 0.78:
        status = "✅ Good — no fine-tuning needed"
    elif auc >= 0.72:
        status = "⚠  Acceptable — supporting agent, ensemble is strong"
    elif auc >= 0.65:
        status = "⚠  Weak individual — acceptable if ensemble >= 0.80"
    else:
